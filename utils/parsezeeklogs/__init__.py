...
class ParseZeekLogs(object):
    ...
    def __init__(self, filepath, batch_size=500, fields=None, output_format=None, ignore_keys=None, meta=None,
                 safe_headers=False):
        if meta is None:
            meta = {}
        ...
        while l.strip().startswith("#"):
            ...
            if l.startswith("#separator"):
                ...
                value = str.encode('\\x09').decode('unicode_escape')
        ...
        # Save mapping of fields to value:
    ...
    def __next__(self):
        ...
        if self.firstRun is True:
            ret_val = self.firstLine
            self.firstRun = False
        else:
            ret_val = self.fd.readline().strip()
        ...
        ret_val = ret_val.split(self.options.get('separator'))

        record = None
        # Make sure we aren't dealing with a comment line
        if len(ret_val) > 0 and not str(ret_val[0]).strip().startswith("#") \
                and len(ret_val) is len(self.options.get("fields")):
            record = OrderedDict()
            # Prepare fields for conversion
            for x in range(0, len(ret_val)):
                if self.safe_headers is True:
                ...
                if self.filtered_fields is None or converted_field_name in self.filtered_fields:
                    ...
                    record[converted_field_name] = ret_val[x]
    ...
    def convert_values(self, data, ignore_keys=None, data_types=None):
    ...
    @staticmethod
    def batch_to_elk(filepath=None, batch_size=500, fields=None, elk_ip="127.0.0.1", index="zeeklogs", meta=None,
                     ignore_keys=None):
